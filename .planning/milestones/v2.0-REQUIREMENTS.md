# Requirements Archive: v2.0 Architecture Correctness & vSAN Max
...
- [x] **NVME-01**: NVMe Memory Tiering toggle available (ESXi 9.x feature)
- [x] **NVME-02**: When enabled, user inputs estimated active memory % (0-100)
- [x] **NVME-03**: When active memory ≤ 50%, tool recalculates required physical DRAM as half the total (1:1 DRAM-to-NVMe ratio)
- [x] **NVME-04**: Tool displays prerequisite notice: requires Class D+ NVMe at 3+ DWPD

### Stretch Cluster

- [x] **STRCH-01**: Stretch cluster configures preferred site and secondary site independently (host count, storage per site)
- [x] **STRCH-02**: Witness node overhead calculated and added to total resource requirements
- [x] **STRCH-03**: Storage calculation models PFTT=1 site mirroring: `calcStorage(totalHosts)` gives total cluster capacity; per-site usable = `safeUsableCapacityTB / 2` (each site holds a full copy). UI displays per-site usable as the effective workload capacity.
- [x] **STRCH-04**: Mutual exclusion enforced: Stretch Cluster + Global Deduplication cannot both be active (UI prevents combination)
- [x] **STRCH-05**: Minimum bandwidth recommendation displayed (cross-site link sizing guidance)

### AI / GPU Workloads

- [x] **GPU-01**: User can input number of GPU-accelerated VMs
- [x] **GPU-02**: User can input vGPU profile (memory per vGPU, GB)
- [x] **GPU-03**: Tool computes additional host RAM required for vGPU overhead (2-3x vGPU memory per host)
